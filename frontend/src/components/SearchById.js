...
const SearchById = () =>{
    ...
    const handlesearch = async()=> {
            try{
              console.log(`http://localhost:3000/api/data/${sinput}`);
                const res = await axios.get(`http://localhost:3000/api/data/${sinput}`);
          ...
          <div>
            ...
            <ul>
              {sres.length > 0 ? (
                ...
                <li key={sres.id}>
                  <strong>Name:</strong> {sres.name}, <strong>ID:</strong> {sres.id},{' '}
                  <strong>Assignee:</strong> {sres.assignee}, <strong>Project:</strong>{' '}
                  {sres.project}, <strong>StartTime:</strong> {sres.startTime}
                </li>
              )}
            </ul>
          </div>
