...
const SearchByAssignee = () =>{
    ...
    const handlesearch = async()=> {
            try{
                const res = await axios.get(`http://localhost:3000/api/data/assign/${sinput}`);
                setsres(res.data);
            ...
            catch(error)
            {
              if (error.response && error.response.status === 404) {
                alert('No record found with the provided assignee.');
              }
              else 
              {
    ...
    return(
        ...
        <label>Enter the assignee name:</label>
